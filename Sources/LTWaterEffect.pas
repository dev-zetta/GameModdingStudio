...
begin
  ...
  begin
    ...
    begin
      NewVal := ((FCurrentPage^[Count - FHeight] +  FCurrentPage^[Count - 1] + FCurrentPage^[Count + 1] + FCurrentPage^[Count + FHeight]) shr 1) - FNewPage^[Count];
      if NewVal < 0 then
...
begin
  if FActive then
    FHeightMap[FPage][Random(FWidth)*FHeight+Random(FHeight)] := FRainAmount;
end;
...
var C,O,oX,oY,X1,Y1,X,Y : Integer;
    P     : PRGB32;
    Dst : PRGB32;
begin
  Dst := FDst;
